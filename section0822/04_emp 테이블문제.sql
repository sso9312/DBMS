...
select ename, hiredate, deptno from emp order by deptno, hiredate; --asc 생략가능
...
select ename 이름, job 직급, sal 급여 --as생략가능
...
select ename, hiredate, sal, comm
     ...
     , (sal*12+nvl(comm, 0)) * 0.1 as 인상된금액 --띄어쓰기 주의
from emp
where trunc((sysdate-hiredate)/365) >= 20;
