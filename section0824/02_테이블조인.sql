...
--수갈신청한 학생들의 학번, 과목코드, 이름, 이메일 조회하시오
...
select count(*) from tb_sugang; --테이블확인
select count(*) from tb_student; --테이블확인
select count(*) from tb_gwamok; --테이블확인
...
--소속테이블명 정확히 명시한 예 
select SU.hakno, GW.gcode, uname, email, gname, ghakjum --에러
from tb_sugang SU inner join tb_student ST
on SU.hakno = ST.hakno inner join tb_gwamok GW
on SU.gcode = GW.gcode;
